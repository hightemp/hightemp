...
- Linux
        ...
        - Общее (General)
            ...
            - DNS
                ...
                - Default Address Selection
                    ...
                    - RFC 3484 on Linux — статья Ulrich Drepper о реализации выбора адресов в Linux: сортировка `getaddrinfo()` в glibc, выбор адреса источника в ядре, проблемы IPv6 site-local/ULA, таблицы `label`/`precedence` и `/etc/gai.conf`. https://www.akkadia.org/drepper/linux-rfc3484.html
                - gai.conf
    ...
    - **Системное администрирование и Утилиты (System Administration and Utilities)**
        ...
        - Блокировки; flock
            ...
            - flock — блокировки в shell скриптах — русскоязычная статья о защите shell-скриптов от повторного запуска: race condition при обычном `.lock`-файле, блокировка file descriptor через `flock -n`, запись PID, cleanup через `trap` и закрытие дескриптора. https://r4ven.me/automation/flock-blokirovki-v-shell-skriptah/
        - Логирование (Logging)
            - Общее (General)

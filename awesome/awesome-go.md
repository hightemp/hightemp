...
- Scraping html; Извлечение статей
  - Go-Trafilatura — пакет и CLI на Go для бесшовной загрузки, парсинга и извлечения содержимого веб-страниц (метаданные, основной текст, комментарии) с сохранением части форматирования; порт Trafilatura, актуальный до v2.0.0, с HTML как основным форматом вывода (также TXT/JSON), пакетной обработкой (URL, RSS, sitemap) и настраиваемыми fallback-экстракторами (go-readability, go-domdistiller). https://github.com/markusmobius/go-trafilatura
  - go-readability — библиотека на Go, реализующая алгоритмы Mozilla Readability для извлечения основного содержимого веб-страниц с удалением «шума», поддержкой вывода в HTML/Markdown, извлечением метаданных и наличием CLI-инструмента. https://github.com/mackee/go-readability
- JSON
  - Sonic — сверхбыстрая библиотека для сериализации и десериализации JSON в Go, ускоренная технологиями JIT (компиляция "на лету") и SIMD (векторные инструкции), поддерживающая привязку объектов во время выполнения без генерации кода и полный набор API для манипуляции JSON-значениями. https://github.com/bytedance/sonic
...
- Инструменты
  ...
  - gogrep — архивированный CLI для поиска Go-кода по syntax trees: поддерживает wildcard patterns, фильтрацию найденных узлов, подстановки и запись измененного Go-кода обратно. https://github.com/mvdan/gogrep
- Кроскомпиляция
  ...
  - go-builder - CLI для декларативной сборки Go-проектов через `.gobuilder.yml`: описывает targets, `ldflags`, build tags, переменные линковки, CGO-настройки, Docker-сборку, dry-run и проверку статической линковки бинарников. https://github.com/pablolagos/go-builder
  - goreleaser-cross - Docker-образ для CGO-кросс-компиляции Go-проектов с GoReleaser: включает toolchain для Linux, Darwin и Windows, поддерживает multi-arch образы, настройку `CC`, `CXX`, `CGO_ENABLED`, `PKG_CONFIG_*` и custom sysroot для сборок под целевые платформы. https://github.com/goreleaser/goreleaser-cross
- javascript
...
- это бесплатный онлайн‑сервис, который анализирует код Go‑проекта по его репозиторию и формирует отчёт о качестве кода на основе различных статических проверок и стиля. https://goreportcard.com/
...
- Описание языка
  - Что нового
    - 1.26
      ...
      - Go 1.26 вышел, пройдемся по всем изменениям… https://habr.com/ru/articles/995906/
    - 1.25
  ...
  - Пакеты стандартной библиотеки
    ...
    - sync
      ...
      - sync.Mutex
        ...
        - Go EP 15: sync.Mutex - Normal and Starvation Mode https://blog.devtrovert.com/p/go-sync-mutex-normal-and-starvation?hide_intro_popup=true
      - sync.Cond
...
- Кросс-компиляция
  ...
  - Cross-compiling a CGO project using Goreleaser — практический пример настройки `goreleaser-cross` для CGO-кросс-компиляции Go CLI в GitLab CI: глобальный `CGO_ENABLED=1`, per-target `CC`/`CXX` для Darwin, Linux и Windows и сборка через Docker-образ `goreleaser/goreleaser-cross`. https://www.jvt.me/posts/2023/02/24/goreleaser-cgo-cross-compile/
...
- Безопасность
  - Supply chain; Зависимости
    ...
    - Hijackable Go Module Repositories — исследование тысяч Go-модулей, уязвимых к repojacking после переименования или удаления GitHub-аккаунтов. https://www.vulncheck.com/blog/go-repojacking
    - Go Supply Chain Attack: Malicious Package Exploits Go Module Proxy Caching for Persistence — BoltDB-typosquat, RCE-backdoor и сокрытие вредоносного кода через изменение Git tag после кеширования в Go Proxy. https://socket.dev/blog/malicious-package-exploits-go-module-proxy-caching-for-persistence
    - wget to Wipeout: Malicious Go Modules Fetch Destructive Payload — Go-модули с обфусцированными загрузчиками destructive malware для уничтожения данных. https://socket.dev/blog/wget-to-wipeout-malicious-go-modules-fetch-destructive-payload
    - Typosquatted Go Packages Deliver Malware Loader Targeting Linux and macOS Systems — семь поддельных модулей с общими техниками обфускации и загрузки malware. https://socket.dev/blog/typosquatted-go-packages-deliver-malware-loader
    ...
    - Turn Dependabot Off — сравнение шумных dependency alerts с reachability-анализом `govulncheck` и пример безопасного CI, тестирующего актуальные зависимости без автоматического обновления production-графа. https://words.filippo.io/dependabot/
    - Inspecting the Source of Go Modules — почему mutable Git tags и интерфейс code host могут показывать не тот код, который попал в Go Module Proxy, и как проверять канонический source через checksum database. https://words.filippo.io/go-source/
    - `go.sum` Is Not a Lockfile — роли `go.mod`, `go.sum` и Go Checksum Database в разрешении версий и проверке целостности зависимостей. https://words.filippo.io/gosum/
    - I’m Independently Verifying Go’s Reproducible Builds — воспроизводимость Go toolchain, checksum transparency log, bootstrap chain и снижение риска Trusting Trust атак. https://www.agwa.name/blog/post/verifying_go_reproducible_builds
  ...
  - Файловая система
    ...
    - `filepath.Ext` Notes — регистр, точка в возвращаемом расширении и другие особенности `filepath.Ext`, позволяющие обходить blacklist-проверки типов файлов. https://parsiya.net/blog/2018-11-10-filepath.ext-notes/
  - Загрузка файлов
  ...
  - HTTP
    ...
    - Go and enhance your calm — неправильное использование Go HTTP/2 client, превращающее обычный трафик в PING flood. https://blog.cloudflare.com/go-and-enhance-your-calm/
    - So you want to expose Go on the Internet — TLS, timeouts и защита Internet-facing Go-сервера; конкретные настройки 2016 года следует адаптировать к современному `crypto/tls`. https://blog.cloudflare.com/exposing-go-on-the-internet/
  ...
  - CORS и browser security policy
    ...
    - Reconfigurable CORS Middleware with `jub0bs/cors` — разбор критической ошибки в radix tree, разрешавшей недоверенные origins, и пример того, почему даже 100% code coverage не гарантирует безопасность. https://jub0bs.com/posts/2024-05-14-reconfigurable-cors-middleware/
  ...
  - DoS и исчерпание ресурсов
    ...
    - Diving into Go’s HTTP Server Timeouts — взаимодействие `ReadHeaderTimeout`, `ReadTimeout`, `WriteTimeout`, `IdleTimeout` и `http.TimeoutHandler`, включая поведение за load balancer. https://adam-p.ca/blog/2022/01/golang-http-server-timeouts/
  ...
  - URL parsing и SSRF
    ...
    - Preventing Server-Side Request Forgery in Golang — проверка адреса после DNS resolution через `net.Dialer.Control` для защиты от private IP и DNS rebinding; код 2019 года следует адаптировать к современным сетевым API. https://www.agwa.name/blog/post/preventing_server_side_request_forgery_in_golang
    - Mitigating SSRF Vulnerabilities in Go. Part 1 — уязвимый Gin endpoint, `gosec`, обход denylist через IP и обход allowlist через HTTP redirect. https://xvnpw.github.io/posts/mitigating_ssrf_vulnerabilities_in_go_a_practical_guide_part_1/
    - Mitigating SSRF Vulnerabilities in Go. Part 2 — защита redirects и DNS rebinding через `safeurl`, настройка `CheckRedirect`, egress proxy и network policies. https://xvnpw.github.io/posts/mitigating_ssrf_vulnerabilities_in_go_a_practical_guide_part_2/
  - XML и SAML
  ...
  - Аудит кода
    ...
    - Golang code review notes — traversal, goroutine leaks, `unsafe`, форматирование URL и платформенные различия, важные при security review. https://www.elttam.com/blog/golang-codereview
    ...
    - Security Vulnerabilities in Smallstep PKI Software — аудит написанного на Go CA: JSON injection в certificate templates, неправильное доверие JWT и client-side enforcement серверных ограничений. https://www.agwa.name/blog/post/security_vulnerabilities_in_smallstep
    - Security Review of CFSSL Signer Code — отслеживание данных из CSR, `SignRequest` и certificate profiles до SAN, Basic Constraints и расширений сертификата. https://www.agwa.name/blog/post/security_review_of_cfssl_signer_code
  ...
  - CSRF
    ...
    - Cross-Site Request Forgery — глубокий разбор same-site и same-origin, `Origin`, `Sec-Fetch-Site`, SameSite cookies, HSTS и дизайна Go `http.CrossOriginProtection`. https://words.filippo.io/csrf/
  - Пароли
  ...
  - SSH
    - Simple SSH Harvester in Go — проверка SSH host keys и сертификатов через `ssh.CertChecker`, `IsHostAuthority` и `HostKeyFallback`; API и зависимости примера 2017 года следует перепроверять. https://parsiya.net/blog/2017-12-29-simple-ssh-harvester-in-go/
  - TLS, mTLS и PKI
  ...
  - Криптография
    ...
    - Go Assembly Mutation Testing — mutation testing constant-time cryptographic assembly для обнаружения непокрытых carry flags, ветвей выбора и ошибок оптимизированного кода. https://words.filippo.io/assembly-mutation/
    - A Cryptographic Near Miss — root-cause analysis ошибки `crypto/elliptic` в Go 1.20.1, возникшей из сочетания assembly, incomplete formulas и недокументированного требования к scalar. https://words.filippo.io/near-miss/
  - Генераторы токенов
  ...
  - Fuzzing
    ...
    - Go or No Go: Differential Fuzzing of Native and C Libraries — сравнение Go/C реализаций libcrypto, libssl, libpng и zlib и security impact parser differentials. https://mweissbacher.com/2023/05/25/go-or-no-go/
    - Coverage Guided Fuzzing in Go — исторический обзор `go-fuzz`, libFuzzer и compiler instrumentation до появления native fuzzing. https://alexplaskett.github.io/coverage-guided-fuzzing-golang/
    - Golang Fuzzing: Key Improvements in Go 1.19 — 8-bit counters, value profiling, string comparison interception и семь найденных багов. https://www.code-intelligence.com/blog/golang-fuzzing-1.19
    - Fuzz Testing in Go: Discovering Vulnerabilities and Analyzing CVE-2022-43677 — практический fuzz test и воспроизведение уязвимости free5GC. https://free5gc.org/blog/20230809/main/
    - The HTTP Garden — differential fuzzing потоков HTTP/1.1 и parser discrepancies, включая Go `net/http`. https://arxiv.org/abs/2405.17737
  ...
  - Runtime, toolchain и memory safety
    ...
    - Golang heap corruption during garbage collection — историческая эксплуатация GC-багa как use-after-free в Go 1.1. https://blog.stalkr.net/2013/06/golang-heap-corruption-during-garbage.html
    ...
    - How My Minimal, Memory-Safe Go rsync Steers Clear of Vulnerabilities — сравнение двенадцати CVE в C rsync с Go-реализацией, включая bounds checks, zero initialization, symlink races и защиту через `os.Root`. https://michael.stapelberg.ch/posts/2026-05-24-minimal-memory-safe-go-rsync-vulns/
    - Safe Use of `unsafe.Pointer` — поиск опасных преобразований через `go vet` и `checkptr`, pointer arithmetic и чтение за границами структуры; команды в статье 2019 года следует адаптировать к актуальному Go. https://mdlayher.com/blog/safe-use-of-unsafe-pointer/
  ...
  - Reverse engineering
    ...
    - Golang Reverse Engineering Tips — stripped binaries, static linking, PCLN и анализ через IDA Pro. https://www.travismathison.com/posts/Golang-Reverse-Engineering-Tips/

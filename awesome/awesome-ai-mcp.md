...
- Память, код и контекст
    - Память и knowledge graph
        ...
        - Memory Server — официальный MCP-сервер для постоянной памяти Claude через локальный knowledge graph: хранит entities, relations и atomic observations, даёт инструменты `create_entities`, `create_relations`, `add_observations`, `read_graph`, `search_nodes`, `open_nodes`, поддерживает запуск через Docker или `npx` и настройку `MEMORY_FILE_PATH` для JSONL-хранилища. https://www.npmjs.com/package/@modelcontextprotocol/server-memory
        - agentmemory — persistent memory/MCP-сервер для AI coding agents: сохраняет контекст через hooks, MCP или REST API, шарит память между Claude Code, Codex CLI, Cursor, Gemini CLI, OpenCode и другими клиентами, использует локальные embeddings и не требует внешней БД. https://github.com/rohitg00/agentmemory
    - Semantic search и RAG по кодовой базе
    ...
    - Code intelligence, LSP и рефакторинг
        - SocratiCode — open-source codebase context engine/MCP-сервер для AI coding agents: локально индексирует большие репозитории через Qdrant/Ollama, даёт hybrid semantic + BM25 search, AST-aware chunks, dependency graph, symbol-level impact analysis, searchable API/schema/infra artifacts и shared index для нескольких агентов; поддерживает Claude Code, Cursor, VS Code Copilot, Codex CLI, Gemini CLI, OpenCode и другие MCP-хосты. https://github.com/giancarloerra/SocratiCode
        - Serena — MCP toolkit для coding agents с IDE-подобными инструментами: symbol-aware code retrieval, editing, refactoring и memory; работает через LSP backend для 40+ языков или через JetBrains plugin и подключается к Claude Code, Codex, Cursor, VS Code, JetBrains IDEs и другим MCP-клиентам. https://github.com/oraios/serena

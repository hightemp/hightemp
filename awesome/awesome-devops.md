...
- Killercoda — платформа интерактивных браузерных сценариев с изолированными временными виртуальными машинами для Linux, Kubernetes, контейнеров, DevOps и networking; поддерживает автоматическую проверку шагов и подсказки встроенного помощника Brain, а Ubuntu-окружения включают Docker и Podman. https://killercoda.com/
...
- Kubernetes
    - Миграция приложения из Docker Compose в Kubernetes. Как, зачем и с какими проблемами я столкнулся https://habr.com/ru/articles/688558/
- pacemaker
...
- Инструментов наблюдаемости (observability tools)
    ...
    - Стандарты и фреймворки сбора телеметрии
        - OpenTelemetry
            ...
            - Node.js
                - OpenTelemetry Node.js - Getting Started with Tracing, Logs, and Metrics https://signoz.io/opentelemetry/nodejs/
...
- Мониторинг
    - Prometheus
        ...
        - TSDB
            ...
            - Prometheus TSDB (Part 3): Memory Mapping of Head Chunks from Disk — разбор memory-mapped чанков Head-блока: формата на диске, чтения и восстановления после WAL replay; показано влияние подхода на потребление памяти и время запуска. https://ganeshvernekar.com/blog/prometheus-tsdb-mmapping-head-chunks-from-disk/
            - Prometheus TSDB (Part 4): Persistent Block and its Index — разбирает persistent blocks на диске и их индекс: структуру блока, `meta.json`, `chunks`, `index` и `tombstones`, а также создание блоков из Head и их компакцию. https://ganeshvernekar.com/blog/prometheus-tsdb-persistent-block-and-its-index/

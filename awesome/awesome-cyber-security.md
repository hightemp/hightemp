...
- PentestCode — автономный терминальный AI-агент для пентестинга на базе OpenCode с 13 специализированными агентами, общим сохраняемым состоянием проверки, встроенными парсерами security-инструментов и поддержкой 20+ LLM-провайдеров; автоматизирует этапы от разведки до отчёта. Использовать только для CTF, лабораторий и разрешённых проверок. https://github.com/s0ld13rr/pentestcode
...
- ARES — автономная open-source платформа red-team engagement и непрерывной security-валидации (FastAPI + React): автоматическое построение attack graph (DAG), маппинг на MITRE ATT&CK, goal-directed планирование (в т.ч. optional LLM-агенты Claude/OpenAI/Ollama), scope firewall, OPSEC-контроль, шифрованное хранилище (vault) и отчёты; с MCP-шлюзом для Cursor/Claude Desktop/Windsurf. Использовать только для авторизованных проверок. https://github.com/Mafifrizi/ARES
...
- Уязвимости; CVE; write-up
    ...
    - Hacking OpenAI — write-up о цепочке уязвимостей heap overflow и SSO misconfiguration, которая позволила получить доступ к аккаунтам сотрудников OpenAI и внутренним репозиториям; с описанием ответственного раскрытия. https://www.hacktron.ai/blog/hacking-openai
    - HEIF Heist — исследование Hacktron об удалённой эксплуатации production-пайплайнов обработки изображений HEIF, HEIC и AVIF через уязвимости в декодерах и связанных компонентах. https://heif-heist.com/
- TOCTOU
    - Time-of-Check to Time-of-Use — краткое русскоязычное введение в TOCTOU с примерами изменения файла или другого ресурса между проверкой и использованием. https://www.securityvision.ru/education/cyberwiki/t/time-of-check-to-time-of-use-toctou/
    - Race Condition в веб-приложениях: три типа уязвимости и как их находить — практический русскоязычный разбор TOCTOU и других web race conditions на промокодах, платежах, OTP и параллельных запросах с примерами кода и способами защиты. https://habr.com/ru/articles/1044830/
    - CWE-367: Time-of-check Time-of-use Race Condition — каноническая справка MITRE с определением, примерами уязвимого кода, последствиями, способами обнаружения и мерами защиты. https://cwe.mitre.org/data/definitions/367.html
    - Race Conditions — материал OWASP о TOCTOU и других состояниях гонки в приложениях: race window, типовые сценарии, поиск через тестирование и code review, исправление атомарными операциями и синхронизацией. https://owasp.org/www-community/pages/vulnerabilities/race_conditions
    - Race Conditions — учебный раздел PortSwigger Web Security Academy о web TOCTOU, limit overrun и многошаговых состояниях гонки с методикой обнаружения, техниками синхронизации запросов и практическими лабораториями. https://portswigger.net/web-security/race-conditions
- Аппаратная безопасность и firmware
    - Руткиты на основе BIOS. Часть 1 — исторический практический разбор подготовки среды для исследования legacy BIOS: извлечение Phoenix BIOS ROM из VMware 7, подключение IDA к встроенному GDB server, модификация образа и проверка bootkit-кода, выполняющегося до загрузки ОС; описанные техники предшествуют современному подписанному UEFI firmware. https://habr.com/ru/companies/otus/articles/513588/
    - Руткиты на основе BIOS. Часть 2 — продолжение legacy BIOS PoC: внедрение и релокация assembler-кода через модуль распаковки Phoenix BIOS, перехват INT 13h, интеграция VBootkit и патчинг Windows bootloader/kernel для повышения привилегий выбранного процесса. https://habr.com/ru/companies/otus/articles/515158/
- DevSecOps

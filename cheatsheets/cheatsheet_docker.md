...
Быстрый способ для Docker Compose v2:

```bash
docker compose ls -a
```

Команда выводит все compose-проекты, включая остановленные. В колонке `Config Files` можно увидеть путь к `compose.yaml` / `docker-compose.yml`, а по нему понять папку проекта.
...
### Как работает ENTRYPOINT и CMD

В таблицах ниже показано, какая команда выполняется для различных ENTRYPOINT / CMD комбинаций:

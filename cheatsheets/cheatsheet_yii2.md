...
Чтобы отключить использование сессий для конкретного контроллера, вы можете переопределить метод `behaviors()` в вашем контроллере:
...
public function behaviors()
{
    return [
        'sessionControl' => [
            'class' => 'yii\filters\SessionControl',
...
Это отключит автоматическое начало сессии для всех действий в этом контроллере.

2. Отключение куки:

Для отключения куки вы можете использовать компонент `response` в вашем контроллере:
...
public function init()
{
    ...
    Yii::$app->response->headers->set('Set-Cookie', '');
}
```

Это удалит все куки и предотвратит установку новых для этого контроллера.

3. Комбинированный подход:

Вы можете объединить оба метода в одном контроллере:

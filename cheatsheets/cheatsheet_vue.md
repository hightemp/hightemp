## Постфиксы в событиях Vue.js: Полный список и объяснение

Постфиксы в событиях Vue.js играют важную роль в управлении поведением и потоком событий в приложении. Они добавляются к имени события и модифицируют его обработку. 

*   **.stop**: Останавливает всплытие события. Предотвращает дальнейшее распространение события по DOM-дереву к родительским элементам.
*   **.prevent**: Предотвращает выполнение действия браузера по умолчанию, связанного с событием. Например, предотвращает переход по ссылке при клике или отправку формы при нажатии Enter.
*   **.capture**: Обрабатывает событие на этапе захвата. Событие сначала обрабатывается на самом верхнем элементе в DOM-дереве и затем спускается вниз к целевому элементу. 
*   **.self**: Обрабатывает событие только если целевой элемент совпадает с элементом, на котором зарегистрирован обработчик события. 
*   **.once**: Обрабатывает событие только один раз. После первого срабатывания обработчик события автоматически удаляется.
*   **.passive**: Указывает браузеру, что обработчик события не будет вызывать `preventDefault()`. Позволяет браузеру оптимизировать прокрутку страницы на мобильных устройствах. 
...
<!-- Обработка события клика на этапе захвата -->
